...
describe("POST" + UNINCORPORATED_BUSINESS_ADDRESS_MANUAL, () => {
    ...
    it("should return status 400", async () => {
        const res = await router.post(BASE_URL + UNINCORPORATED_BUSINESS_ADDRESS_MANUAL)
            .send({ addressPropertyDetails: "abc", addressLine1: "pqr", addressLine2: "abc", addressTown: "abc", addressCounty: "abcop", addressCountry: "AbcdefghijklmnopqrstuvwxAbcdefghijklmnopqrstuvwxaaA", addressPostcode: "MK9 3GB" });
        expect(res.status).toBe(400);
