...
export const validateUpdatesWithoutDate = (req: Request, acspFullProfile: AcspFullProfile, acspUpdatedFullProfile: AcspFullProfile): AcspFullProfile => {
    const changeFlag = JSON.stringify(acspFullProfile) !== JSON.stringify(acspUpdatedFullProfile);
    const session: Session = req.session as any as Session;
    if (changeFlag && session.getExtraData(ACSP_UPDATE_CHANGE_DATE.NAMEOFBUSINESS) === null &&
    ...
    if (changeFlag && session.getExtraData(ACSP_UPDATE_CHANGE_DATE.WHEREDOYOULIVE) === null &&
    ...
    if (changeFlag && session.getExtraData(ACSP_UPDATE_CHANGE_DATE.CORRESPONDENCEADDRESS) === null &&
    ...
    if (changeFlag && session.getExtraData(ACSP_UPDATE_CHANGE_DATE.REGOFFICEADDRESS) === null &&
