...
export const get = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const previousPage: string = addLangToUrl(BASE_URL + UPDATE_ACSP_CHANGE_DETAILS, lang);
        const currentUrl: string = BASE_URL + UPDATE_CORRESPONDENCE_ADDRESS_LOOKUP;
        ...
        res.render(config.AUTO_LOOKUP_ADDRESS, {
            ...
            currentUrl,
            payload,
            correspondenceAddressManualLink: addLangToUrl(BASE_URL + UPDATE_CORRESPONDENCE_ADDRESS_MANUAL, lang)
        });
    } catch (err) {
        next(err);
    }
        ...
        if (!errorList.isEmpty()) {
            ...
            addressLookUpService.getAddressFromPostcodeUpdateJourney(req, postcode, inputPremise, acspUpdatedFullProfile, false,
                ...
                try {
                    ...
                    logger.error("POST " + BASE_URL + UPDATE_CORRESPONDENCE_ADDRESS_LOOKUP);
                    next(err);
                }
            }).catch(() => {
