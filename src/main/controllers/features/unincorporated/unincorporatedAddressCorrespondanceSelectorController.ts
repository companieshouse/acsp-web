...
import { BASE_URL, TYPE_OF_BUSINESS, UNINCORPORATED_CORRESPONDENCE_ADDRESS_CONFIRM, UNINCORPORATED_NAME_REGISTERED_WITH_AML, UNINCORPORATED_WHAT_IS_THE_CORRESPONDENCE_ADDRESS, UNINCORPORATED_WHAT_IS_YOUR_ROLE } from "../../../types/pageURL";
...
export const post = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const selectedOption = req.body.addressSelectorRadio;
        const session: Session = req.session as any as Session;
        const acspData: ACSPData = session?.getExtraData(USER_DATA)!;
