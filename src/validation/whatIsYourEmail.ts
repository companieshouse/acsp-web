    ...
    body("whatIsYourEmailInput").trim().if(body("whatIsYourEmailRadio").equals("A Different Email"))
        ...
        .isEmail().withMessage("emailFormatIncorrect").custom((input, { req }) => {
            ...
            if (acspDetails) {
                ...
                if (existingEmail && normalisedInput === existingEmail) {
                    throw new Error("emailNoChangeUpdateAcsp");
                }
            } return true;
        }),

    body("whatIsYourEmailRadio", "noEmail").notEmpty().custom((value, { req }) => {
        ...
        if (acspDetails) {
            ...
            if (existingEmail && normalisedInput.toLowerCase() === existingEmail.toLowerCase()) {
