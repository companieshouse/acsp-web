...
import { UPDATE_ACSP_DETAILS_BASE_URL, UPDATE_YOUR_ANSWERS, CANCEL_AN_UPDATE, UPDATE_CHECK_YOUR_UPDATES } from "../../../../src/types/pageURL";
...
describe("GET " + CANCEL_AN_UPDATE, () => {
    ...
    it("should redirect to the your updates page when the return query param is your-updates", async () => {
        const response = await router.get(UPDATE_ACSP_DETAILS_BASE_URL + CANCEL_AN_UPDATE + "?return=your-updates");
        expect(response.status).toBe(302);
        expect(mocks.mockUpdateAcspAuthenticationMiddleware).toHaveBeenCalled();
