...
function searchCountries () {
    ...
    const ul = document.getElementById("autocomplete__listbox");

    // Set the CSS for ul tag
    ul.classList.remove("autocomplete__menu--hidden");
    ul.classList.add("autocomplete__menu--visible");

    // Get the text from the input element
    const inputValue = inputBox.value.trim().toLowerCase();

    // Filter the country list to only show countries with that match the input value
    ...
    for (var i = 0; i < filteredList.length; i++) {
        ...
        li.textContent = filteredList[i];
        li.id = `autocomplete-default__option--${i}`;
